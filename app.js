...
const token = "OTY0MTgwNzExODQ0MjI5MTYw.Ylg5Qw.hbT04mDYZeewOmItzlaM0z0U89M"; // Bot token
const {
    Permissions
} = require('discord.js');
...
client.on("messageCreate", async (message) => {
    ...
    for (let i = 0; i < flaggedWords.length; i++) {
        if (message.content.toLowerCase().includes(flaggedWords[i]) && message.channel.id != "929055765040357409") {
            message.delete()
            break
        }
    }
    if (message.content.startsWith(".createKey")) {
        if (message.channelId == "929055765040357409") {
            ...
            const toRep = new MessageEmbed()
                .setColor('#ff2e2e')
                .setTitle("Unable to create key")
                .setDescription("You can only create keys in the <#929055765040357409> channel.")
            await message.reply({
                embeds: [toRep]
            })
        }
...
client.on("messageUpdate", (oldMessage, newMessage) => {
    for (let i = 0; i < flaggedWords.length; i++) {
        if (newMessage.content.toLowerCase().includes(flaggedWords[i]) && newMessage.channel.id != "929055765040357409") {
            newMessage.delete()
            break
        }
    }
